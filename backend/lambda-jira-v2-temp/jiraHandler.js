...
async function importFromJira(event) {
    ...
    const { jiraUrl, apiToken, email, issueKeys, team } = body;
    ...
    if (!issueKeys || !Array.isArray(issueKeys) || issueKeys.length === 0) {
        return (0, response_1.errorResponse)('Se requiere al menos un issue key para importar', 400);
    }
    try {
        console.log('[2] Preparando autenticación con Jira...');
        const auth = Buffer.from(`${email}:${apiToken}`).toString('base64');
        console.log(`[3] Importando ${issueKeys.length} issues seleccionados...`);
...
async function fetchJiraIssues(jiraUrl, auth, jql) {
    ...
    while (pageCount < MAX_PAGES) {
        ...
        const url = `${jiraUrl}/rest/api/3/search?jql=${encodeURIComponent(jql)}&startAt=${startAt}&maxResults=${maxResults}&fields=${fields}`;
